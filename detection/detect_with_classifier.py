...
#load the input image from disk, resize it such that it has the has the supplied width and then grab its dimensions
orig = cv2.imread(args["image"])
orig = imutils.resize(orig, width = WIDTH)
(H, W) = orig.shape[:2]

#initialize the iamge pyramid
pyramid = image_pyramid(orig, scale = PYR_SCALE, minSize = ROI_SIZE)

#initialize two lists, one to hold the ROIs generated from the image pyramid and sliding window and another list used to store the
...
for (i, p) in enumerate(preds):
    ...
    #filter our weak detections by ensuring the predicted probability is greater than the minimum probability
    if prob >= args["min_conf"]:
        #grab the bounding box associated with the prediction and convert the coordinates
        box = locs[i]

        #grab the list of predictions for the label and add the bouding box and probability to the list
        L = labels.get(label, [])
        L.append((box, prob))
        labels[label] = L
...
for label in labels.keys():
    ...
    #show the results "before" applying non-maxima supression, then clone the image again so we can display the results "after" applying non-maxima supression
    cv2.imshow("Before", clone)
    cv2.waitKey()
    clone = orig.copy()

    #extract the bounding boxes and associated prediction probabilities then apply non-maxima suppresion
    ...
    for (startX, startY, endX, endY) in boxes:
        ...
        cv2.putText(clone, label, (startX,y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 0), 2)

    #show the ouput after apply non-maxima suppression
